...
def get_understat_fixture_data(league_name="DEU Bundesliga 1", start_history_year=2017):
    """
    Fetches historical Understat fixture data dynamically from a specified start year
    up to the most recent season available, including detailed fixture information.
    ...
    print(f"Starting to fetch Understat {league_name} fixture data from {start_history_year}-{start_history_year+1}...")
    ...
    for year in range(start_history_year, current_year + 2):
        ...
        try:
            under = pb.scrapers.Understat(league_name, season_string)
            fixtures = under.get_fixtures()
            if not fixtures.empty:
                print(f"Successfully fetched {len(fixtures)} fixtures for season {season_string}.")
                all_fixtures_data.append(fixtures)
            else:
                print(f"No fixture data found for season {season_string}. This likely means the season is not yet available or complete. Stopping further attempts.")
                break
        except Exception as e:
            print(f"Error fetching data for season {season_string}: {e}. Stopping further attempts.")
            break
    ...
    if all_fixtures_data:
        fixture_history = pd.concat(all_fixtures_data, ignore_index=True)
        print(f"\nSuccessfully collected Understat data for {len(all_fixtures_data)} seasons.")
        print(f"Total number of Understat fixtures collected: {len(fixture_history)}")
        return fixture_history
    else:
        print("\nNo Understat fixture data could be collected.")
        return pd.DataFrame()
    """
    Fetches historical football fixture data dynamically from a specified start year
    up to the most recent season available.
